...
interface SliderGenreProps {
  type: "movie" | "tv"
  setGenre: Dispatch<SetStateAction<number | undefined>>
  currentGenre: number | undefined
}

export default function SliderGenre(props: SliderGenreProps) {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (response.status !== 200) throw new Error("Error fetching movies on trending")
        const result = GenresSchema.safeParse(response.data)
        if (!result.success) throw new Error("Error parsing movies on trending data")
              ...
              ? (
                genres.map((genre) => (
                  <li
                    ref={genreItemRef}
                    key={genre.id}
                    className={`block flex-shrink-0 w-[120px] md:w-[200px] py-2 text-center text-nowrap bg-red-600 text-white rounded-full snap-center hover:cursor-pointer transition-transform select-none ${currentGenre === genre.id ? "bg-red-800" : ""}`}
